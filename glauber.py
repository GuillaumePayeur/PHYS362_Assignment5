...
################################################################################
# Monte Carlo simulation for 2D ising model using Glauber rule. It does many
...
# Function applying the Glauber rule
def Glauber(states,neighbors,n_runs,J,T):
    # delta_E for flips
    delta_E = 2*states*neighbors
    # Probability of P of switching, and random number
    P = (1/2)*(1-np.tanh(delta_E/(2*J*T)))
...
def step(systems,T,size,n_runs,J):
    ...
    for i in range(size*size):
        ...
        # Updating the states
        systems[:,n1[i],n2[i]] = Glauber(states,neighbors,n_runs,J,T)
    return systems
